...
import com.example.administrator.myapplication.SearchActivity;
...
public class BiuEditText extends EditText {
    ...
    private void update(char last, boolean isUp) {
        ...
        float endY = 0;
        if (isUp) {
            startX = layout.getPrimaryHorizontal(pos) + 140;
            startY = height / 3 * 2;
            endX = startX;
            endY = baseline + ascent;
        } else {
            endX = new Random().nextInt(contentContainer.getWidth()) +50;
            endY = height / 3 * 2;
    ...
    private void init() {
        contentContainer = (ViewGroup) ((SearchActivity) getContext()).findViewById(android.R.id.content);
